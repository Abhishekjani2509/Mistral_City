...
#h-huts{left:14px;bottom:14px;padding:8px;display:flex;gap:6px}
.hut{
...
.x:hover{color:#fff}
.ib{overflow-y:auto;padding:11px 13px}
...
.acts{display:flex;flex-direction:column;gap:5px;padding:10px 13px;border-top:2px solid var(--ink);background:var(--paper2)}
.act{display:flex;align-items:center;gap:9px;width:100%;text-align:left;border:2px solid var(--ink);background:var(--paper);padding:7px 9px;cursor:pointer}
...
.act .an{font-size:12px;font-weight:800;display:block;line-height:1.2}
.act .ad{font-size:10px;color:var(--mute);display:block;margin-top:1px}
.act .ac{margin-left:auto;font-family:"Silkscreen",monospace;font-size:10px}
...
const CITY_HTML = `
  <div id="stage">
    ...
    <div id="hint">drag to pan, scroll to zoom, click a building</div>
    ...
    <div id="connect">
      <div class="cbox card">
        ...
        <div class="bot">
          <div class="ritem" data-go="1"><div><div class="rn">~/dev/mistral-shop</div>
            <div class="rm">next.js, prisma, 19 tests, 1 failing</div></div><span class="rr">Demo</span></div>
          <div class="ritem" data-go="1"><div><div class="rn">Open a local folder&hellip;</div>
            <div class="rm">any repository on this machine</div></div></div>
...
export function mountCity(el, opts = {}) {
  ...
  const AGENTS = {
    ...
    guard:{name:'Guard Cat', verb:'Protect', cost:20, fur:'#3A2B2B', accent:C.R, tool:C.stone,
           desc:'Writes the tests that stop it breaking again'}
  };
  ...
  /* ------------------------------------------------------------
     THE WHOLE MAP ANSWERS TO THE SECURITY LEVEL.
     The Town Hall plot already changed with the level. Everything
     outside it did not, so levelling up read as one building
     twitching in the middle of a static field. These tables
  ...
  /* daylight over the whole board. gloom when nothing is checked, warm when everything is. */
  const LIGHT=[
   {c:'#16241A',a:0.30,op:'multiply'},
   {c:'#2E3A26',a:0.14,op:'multiply'},
   {c:null,a:0,op:null},
   {c:'#FFE7BC',a:0.07,op:'overlay'},
   {c:'#FFD79A',a:0.13,op:'overlay'}
  ];
  ...
  /* what grows out there, per level. same eras as the Town Hall plot. */
  const PROP_VOCAB=[
  ...
  const PROPART={
   ...
   /* clipped shrub in a stone tub. RING level 5 asked for this and it did not
      exist, so two of the waterfront props were silently dropped every render. */
   topiary:[
  ...
  /* ============================================================
     ...
     ============================================================ */
  const ZMIN=1, ZMAX=4;
  /* Zoom is stepped to whole numbers so pixels stay square at rest, but the
     camera eases into each step instead of snapping. One wheel gesture is one
     step: without the cooldown a trackpad flick fires 40 events and slams to 4. */
  function clampCam(){
  ...
  function draw(){
    S.tick++;
    /* Ease the camera toward its target, on elapsed time rather than frames.
       A fixed per-frame factor runs at half speed on a 30Hz projector, which
       is exactly the trap the handoff warns about. Settles exactly on target
    ...
    const z=S.cam.z;
    ctx.imageSmoothingEnabled=false;
    ctx.fillStyle=TILE_THEME[S.worldLevel-1].back; ctx.fillRect(0,0,S.vw,S.vh);
    ...
    const LT=LIGHT[S.worldLevel-1];
    if(LT&&LT.a>0){
      ctx.save();
      ctx.globalCompositeOperation=LT.op; ctx.globalAlpha=LT.a;
      ctx.fillStyle=LT.c; ctx.fillRect(0,0,S.vw,S.vh);
      ctx.restore();
    }
    ...
    if(s.status!=='unknown'){
      ...
      if(showName){
        if(s.kind==='tower'){
          ...
          label(`${s.name}   Level ${s.level} ${L.name}`,px+spr.width*z/2,py-4,z,C.ink,C.paper,L.tone,(s.level/5)*100);
  ...
  function renderIns(s){
    ...
    const rec = st==='broken'?'repair': st==='unknown'?'scout':'guard';
    $('iac').innerHTML=Object.entries(AGENTS).map(([k,a])=>{
      const busy=!!S.busy[s.id], poor=S.energy<a.cost;
      const noop=(k==='repair'&&(st==='healthy'||st==='unknown'))||(k==='guard'&&st==='unknown');
  ...
  /* Pixel tick and cross, written the same way as every other sprite in this
     file: a grid of rows. Emitted as 1px SVG rects in currentColor, so the
     plate behind them sets the colour and there is still no image file. */
  ...
  /* Everything the level touches, in one place. Called by setSecurity, by the
     inspector's Apply-fix button and by setModel, so the world can never drift
     out of step with the Town Hall. */
  function applyWorldLevel(lv,quiet){
    lv=Math.max(1,Math.min(5,lv|0));
    if(lv===S.worldLevel) return lv;
    S.worldLevel=lv;
    rollProps(lv);
    themeTiles(lv);                       // bake now so the swap lands on one frame
    const birds=S.ambient.filter(a=>a.kind==='bird').length;
    const want=[1,2,3,4,6][lv-1];
    if(birds<want) for(let i=birds;i<want;i++)
      S.ambient.push({kind:'bird',x:rnd(i*7.7)*MW*TS,y:60+rnd(i*3.3)*140,
                      vx:0.5+rnd(i)*0.5,ph:i*1.7,by:0});
    else if(birds>want){
      let drop=birds-want;
      S.ambient=S.ambient.filter(a=>(a.kind==='bird'&&drop-->0)?false:true);
    }
    if(!quiet) log(`the whole town shifted to <b>${LEVELS[lv-1].name}</b>, ${LEVELS[lv-1].era.toLowerCase()}`,'good');
    return lv;
  }
  ...
  const SCRIPT={
    ...
    scout:s=>[[0,`scout entering the fog`,'sys'],
      [900,`walking the directory tree`,'code'],
      [2200,`found src/lib/stripe.ts, api/checkout/route.ts`,'code'],
      [3600,`reading the webhook handler`,'code'],
      [4900,`this ground takes payments through Stripe`,''],
      [6000,`mapped 2 systems, 3 connections`,'good']],
    guard:s=>[[0,`dispatching guard agent to <b>${s.name}</b>`,'sys'],
      [900,`listing untested behaviour`,'code'],
      [2100,`3 of 9 routes have no test`,'bad'],
      [3400,`writing tests/api-cart.test.ts`,'code'],
      [5200,`running: pnpm test api`,'code'],
      [6800,`9 passed, coverage 62% to 88%`,'good']]
  };
  ...
  function dispatch(agentKey,target){
    ...
    if(agentKey==='guard'&&target.status==='unknown')return toast('Scout it first.');
    ...
    const cat={agent:agentKey,x:start.x,y:start.y,dir:1,state:'walk',
               say:agentKey==='repair'?'on it':agentKey==='scout'?'let me look':'fortifying',
               sayUntil:S.tick+120};
    S.cats.push(cat);
  ...
  function finish(agentKey,target,cat,scaf,A){
    ...
    if(agentKey==='scout'){
      target.name='Payments District'; target.status='warning'; target.health=79;
      target.blurb='Takes money from customers through Stripe and turns carts into orders.';
      target.files=['src/lib/stripe.ts','src/app/api/checkout/route.ts','src/app/api/webhooks/stripe/route.ts'];
      target.issues=[{w:1,t:'The declined-card path has no test',
        d:'If Stripe rejects a card, nothing in the suite checks what the customer sees.'}];
      target.connections=[...target.connections,'stripe','checkout'];
      setKnow(S.knowledge+2);
      floatText(A.x,A.y-A.h-6,'+2 KNOWLEDGE',C.o);
      HIDDEN.forEach((hh,i)=>setTimeout(()=>{
        hh.grow=0; S.systems.push(hh); MODEL.systems.push(hh);
        const a2=anchor(hh); burst(a2.x,a2.y-10,12,'spark');
        log(`discovered <b>${hh.name}</b>`,'good');
      },420*i+300));
      target.result=`<div class="res-card"><div class="rt">Scout Cat mapped the district</div>
        <div class="rd">Two systems and three connections added to the city. This is where payments happen.</div></div>`;
      log(`fog lifted over <b>Payments District</b>`,'good');
    }

    if(agentKey==='guard'){
      const was=target.health;
      target.health=Math.min(99,target.health+13);
      if(target.status==='warning')target.status='healthy';
      target.issues=[]; S.tt+=3; S.tp+=3; setTests();
      burst(A.x,A.y-A.h*0.6,20,'spark');
      floatText(A.x,A.y-A.h-6,`+${target.health-was}%`,C.y);
      target.result=`<div class="res-card"><div class="rt">Guard Cat added 3 tests</div>
        <div class="rd">Coverage 62% → 88% on this system. A watchtower now covers the district.</div>
        <details><summary>What changed</summary><div class="df">A tests/api-cart.test.ts<br><br>&#10003; 9 passed</div></details></div>`;
      log(`<b>${target.name}</b> fortified, +3 tests`,'good');
    }
  ...
  stage.addEventListener('wheel',e=>{
    ...
    if(now<wheelLock){ wheelAcc=0; return; }       // swallow the momentum tail
    let d=e.deltaY;
    if(e.deltaMode===1) d*=16; else if(e.deltaMode===2) d*=100;
    wheelAcc+=d;
    if(Math.abs(wheelAcc)<80) return;
    const dir=wheelAcc>0?-1:1;
    wheelAcc=0; wheelLock=now+240;                 // one gesture is one step
    const r=cv.getBoundingClientRect();
    zoomTo(S.cam.zt+dir,e.clientX-r.left,e.clientY-r.top);
  },{passive:false});
  ...
  function sceneLayout(lv,halfW){
    ...
    ['scout','repair','guard'].forEach((a,i)=>out.push({t:'a',k:a,x:(i-1)*34+jitter(6),y:14+jitter(7)}));
  ...
  MistralCity=Object.assign(MistralCity,{
    ...
    setModel(json){
      ...
      /* the world dresses itself to match the scan's security result */
      applyWorldLevel(Math.max(1,Math.min(5,
        ((json.city&&json.city.security&&json.city.security.passed)||[]).length||1)),true);
    ...
    setSecurity(passedIds){
      ...
      /* the world changes even before a repository is connected, so the
         control panel is useful on an empty map */
      applyWorldLevel(lv);
      if(!t) return lv;
      t.passed=ids; t.level=lv; t.grow=0.55;
      const a=anchor(t); burst(a.x,a.y-a.h*0.6,26,'spark');
      if(S.selected&&S.selected.id==='tower') renderIns(t);
      return lv;
    },

    /* the five checks, so a host UI can build its own controls */
    checks(){ return CHECKS.map(c=>({id:c.id,name:c.name})) },
    securityLevel(){ const t=sysAt('tower'); return t?(t.level||1):S.worldLevel },
    ...
    state(){ return {health:S.health,energy:S.energy,systems:S.systems.length,
      zoom:S.cam.zt, level:S.worldLevel}; },
    destroy(){
